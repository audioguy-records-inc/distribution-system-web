  ...
  const handleUpload = async (file: File) => {
    try {
      ...
      if (success) {
        ...
        if (track && !track.spatialAudioInfo) {
        ...
        if (track && track.spatialAudioInfo) {
          if (!track.spatialAudioInfo.trackFileList) {
            track.spatialAudioInfo.trackFileList = [];
          }

          track.spatialAudioInfo.trackFileList.push({
            name: success.name,
            filePath: success.filePath,
          });
        }
    ...
    <Container>
      ...
      track.spatialAudioInfo.trackFileList.length > 0 ? (
        <AudioFileWrapper>
          <AudioContainer>
            <AudioName>
              {track.spatialAudioInfo.trackFileList[0].name}
            </AudioName>
            <AudioActions>
              {!readOnly && (
