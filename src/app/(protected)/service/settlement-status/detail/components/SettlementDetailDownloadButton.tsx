...
export default function SettlementDetailDownloadButton() {
  ...
  const prepareSettlementDetailData = () => {
    ...
    const detailData = settlementDetails.map((detail) => ({
      [t("settlementMonth")]: detail.settlementMonth,
      [t("salesMonth")]: detail.salesMonth,
      [t("serviceChannel")]: detail.service,
      UPC: detail.UPC,
      ISRC: detail.ISRC,
      [t("albumName")]: detail.albumTitle,
      [t("trackName")]: detail.trackTitle,
      [t("releaseDate")]: detail.utcReleasedAt,
      [t("agency")]: detail.agencyCompanyName,
      [t("licensorName")]: detail.userDisplayName,
      [t("licensorAccount")]: detail.userAccount,
      [t("albumDistributionCode")]: detail.albumDistributionCode,
      [t("settlementAmount")]: detail.userSettlementFee,
      [t("artist")]: detail.artistNameList.join(", "),
    }));
  ...
  const handleExcelDownload = async () => {
    ...
    if (settlementDetailData) {
      ...
      XLSX.utils.book_append_sheet(workbook, detailWorksheet, t("settlementDetailHistory"));
    }
    ...
    await saveAs(
      blob,
      `${t("settlementDetailHistory")}_${new Date().toISOString().split("T")[0]}.xlsx`,
    );
  ...
  const handleCsvDownload = async () => {
    ...
    saveAs(blob, `${t("settlementDetailHistory")}_${new Date().toISOString().split("T")[0]}.csv`);
